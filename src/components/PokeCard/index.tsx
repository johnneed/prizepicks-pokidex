...
export const PokeCard = ({ pokemon }: PokeCardProps) => {
    ...
    return (
        <div className={styles.pokecard}>
            <article className={isFlipped ? styles["pokecard_show-back"] : styles["pokecard_show-front"]}
                     onClick={() => setIsFlipped(!isFlipped)}>
                <section className={styles.pokecard_front}>
                    <div className={styles.pokecard_name}>{pokemon.name}</div>
                    <div className={styles.pokecard_pic} style={{ backgroundImage: "url('" + image + "')" }} />
                ...
                <section className={styles.pokecard_back}>
                    ...
                    <div className={styles.pokecard_stats}>
                        {pokemon.stats.map(stat => (
                            <div key={stat.stat.name}><span>{stat.stat.name}</span><span>{stat.base_stat}</span>
                            </div>))}
                    </div>
